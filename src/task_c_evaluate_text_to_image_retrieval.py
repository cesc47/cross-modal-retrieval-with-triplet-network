...
def extract_embeddings(dataloader, model, out_size=256):
    model.to('cuda')
    with torch.no_grad():
        model.eval()
...
def main():
    # Load the datasets
    ROOT_PATH = "../../data/"
    TEST_IMG_EMB = ROOT_PATH + "Flickr30k/v2_test_FasterRCNN_features.pkl"
    ...
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=1024, shuffle=False, num_workers=1)
    ...
    # Extract embeddings
    image_embeddings, text_embeddings = extract_embeddings(test_loader, model, out_size)
